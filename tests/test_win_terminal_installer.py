...
class TestTerminalInstaller:
    ...
    def fixture_settings(self, tmp_path_class: Path) -> Settings:
        """Settings instance to be used by the tests in the class."""
    ...
    def fixture_backup(self, terminal_installer: TerminalInstaller) -> Result[str, str]:
    ...
    def fixture_install(self, terminal_installer: TerminalInstaller) -> Result[str, str]:
    ...
    def fixture_uninstall(self, install: Result[str, str], terminal_installer: TerminalInstaller) -> Result[str, str]:  # noqa: ARG002
    ...
    def test_backup_result_ok(self, backup: Result[str, str]) -> None:
        """Check that the function runs and returns Ok."""
        assert is_ok(backup)

    def test_backup_files_match(self, backup: Result[str, str], terminal_installer: TerminalInstaller) -> None:  # noqa: ARG002
        """Check that the backed up files match the target directory."""
        backup_files = [x.name for x in terminal_installer.backup_dir().rglob("*")]
    ...
    def test_install_file_color_scheme(self, install: Result[str, str], terminal_installer: TerminalInstaller) -> None:  # noqa: ARG002
    ...
    def test_install_file_always_show_tabs(
        self,
        install: Result[str, str],  # noqa: ARG002
        terminal_installer: TerminalInstaller,
    ) -> None:
    ...
    def test_uninstall_tabs_setting_reverted(
        self,
        uninstall: Result[str, str],  # noqa: ARG002
        terminal_installer: TerminalInstaller,
    ) -> None:
